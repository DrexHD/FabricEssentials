        ...
        literal.then(
                argument("player", player())
                        ...
                        .executes(ctx -> {
                            ...
                            ctx.getSource().sendSystemMessage(localized("fabric-essentials.commands.ping.other", ctx.getSource(), ServerPlaceholderContext.of(player)));

                            return player.connection.latency();
                        })
        ).executes(ctx -> {
            ServerPlayer player = ctx.getSource().getPlayerOrException();
            ctx.getSource().sendSystemMessage(localized("fabric-essentials.commands.ping.self", ctx.getSource()));
            return player.connection.latency();
        });
    }
}
