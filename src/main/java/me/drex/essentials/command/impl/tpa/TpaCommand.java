...
public class TpaCommand extends Command {
    ...
    private int execute(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        TpaManager.Participants participants = new TpaManager.Participants(player.getUUID(), target.getUUID());
        TpaManager.Direction direction = TpaManager.INSTANCE.getRequest(participants);
        if (direction == this.direction) {
            ctx.getSource().sendFailure(localized("fabric-essentials.commands.tpa.pending", ctx.getSource(), ServerPlaceholderContext.of(target)));
            return FAILURE;
        }
        TpaManager.INSTANCE.addRequest(participants, this.direction);
        ctx.getSource().sendSuccess(() -> localized("fabric-essentials.commands." + this.direction.getTranslationKey() + ".self", ctx.getSource(), ServerPlaceholderContext.of(target)), false);
        target.sendSystemMessage(localized("fabric-essentials.commands." + this.direction.getTranslationKey() + ".victim", target.createCommandSourceStack(), ServerPlaceholderContext.of(player)));
        return SUCCESS;
    }
